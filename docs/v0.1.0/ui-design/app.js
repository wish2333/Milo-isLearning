    ...
    const stagesHtml = stageLabels
      .map((s) => {
        ...
        const color = isActive
          ...
          : 'var(--fg-tertiary)'
        return (
          `<span class="label-stage" style="color: ${color};">${s.label}</span>` +
          (s.id !== 'feynman'
  ...
  function renderStaircase(opts = {}) {
      ...
      const status = allComplete
        ...
        : i < activeConceptIdx
          ...
          : i === activeConceptIdx && activeStage === 'concept'
            ...
            : 'stair--locked'
      cells.push(
        `<li class="stair ${status}" aria-label="Concept ${i + 1}"><span class="stair__label">C${i + 1}</span></li>`,
      )
      cells.push(
        `<li class="stair ${status}" aria-label="Concept ${i + 1}"><span class="stair__label">C${i + 1}</span></li>`,
      )
    ...
    const challengeStatus = allComplete
      ...
      : activeStage === 'challenge'
        ...
        : 'stair--locked'
    cells.push(
      `<li class="stair ${challengeStatus} staircase--challenge" aria-label="Challenge"><span class="stair__label">Chl</span></li>`,
    )
    cells.push(
      `<li class="stair ${challengeStatus} staircase--challenge" aria-label="Challenge"><span class="stair__label">Chl</span></li>`,
    )
    ...
    const feynmanStatus = allComplete
      ...
      : activeStage === 'feynman'
        ...
        : 'stair--locked'
    cells.push(
      `<li class="stair ${feynmanStatus} staircase--feynman" aria-label="Feynman"><span class="stair__label">Fey</span></li>`,
    )
    cells.push(
      `<li class="stair ${feynmanStatus} staircase--feynman" aria-label="Feynman"><span class="stair__label">Fey</span></li>`,
    )
