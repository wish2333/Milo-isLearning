...
> 对应规格说明书：[`Product-Specification.md`](./Product-Specification.md) V1.0
> 本 PRD 聚焦 **V1 MVP** 的可实施需求，V2+ 范围仅作前瞻引用。
...
继承规格书[附录 A 术语表](./Product-Specification.md#附录-a术语表)。本 PRD 额外引入：
...
规格书[第一章 1.1](./Product-Specification.md#11-学习产品的问题) 指出：现有学习产品（视频、阅读、AI 聊天、Quiz 工具）都没有解决一个核心问题——**学习最大的障碍不是知识本身，而是表达成本（Expression Cost）**。
...
- **US-10 [M]**：作为学习者，当我答错时，我想看到解释并自动获得一道**同类型同层级**的新题，而不是被惩罚或卡住。
...
| 约束 | - MVP 编译产物为**单个 Module**（多 Module 推迟）<br>- Module 内 Concept 数量限定 **2-5 个**（过少无法形成 Module Feynman 价值，过多超出 MVP 编译能力）<br>- 每个 Concept 的 QuizSeries 包含 **8-15 道 Quiz**，覆盖 Recognition / Discrimination / Application 三个层级（MVP 不含 Association，详见 §10.2）<br>- 编译总耗时 ≤ 60 秒（含全部 Agent 调用）<br>- 编译过程中显示阶段化进度 |
...
| 验收 | AC1: Choice 题一次点击即可提交，提交后立即显示反馈<br>AC2: Sorting 题支持拖拽与点击两种方式调整顺序<br>AC3: Fill Blank 题对大小写、首尾空格、全半角不敏感<br>AC4: 同一 Concept 内前 2 题为 Level 1 Choice<br>AC5: 表达层级分布统计符合约束<br>AC6: 单题反馈延迟 < 1.5 秒（Feedback Agent 响应） |
...
|----|------|
| 优先级 | **Must** |
| 描述 | 答对进入下一题；答错显示解释并由 Quiz Agent 生成同类型同层级新题供重试 |
| 输入 | 用户答案 + 原 Quiz |
| 输出 | `score` / `gaps` / `next_action` / 反馈文案 / （如 retry）新生成的 Quiz |
| 约束 | - 答对（score ≥ 80）→ `next_action=advance`，进入下一题<br>- 答错（score < 80）→ `next_action=retry`，**保留原题记录**，生成新题（同 concept、同 ladderLevel、同 interactionType、同 expressionLevel），干扰项必须更换<br>- **同一 Concept 内连续答错同一题不超过 3 次**：第 3 次仍错则强制 advance 并标记该 Concept 为"待复习"<br>- 反馈文案 ≤ 50 字，鼓励性语气（"差一点！关键在于……"）<br>- 解释（explanation）需同时说明"为什么对"和"为什么错"<br>- 不惩罚、不扣分、不显示失败感强的视觉元素（红色叉号慎用，详见 §10 UI） |
| 验收 | AC1: 答对直接进入下一题，无中间环节<br>AC2: 答错显示解释 + 自动出现新题<br>AC3: 新题与原题 concept/层级/类型完全相同，但题干与干扰项不同<br>AC4: 连续 3 次答错后强制进入下一题<br>AC5: 反馈文案符合鼓励性语气要求 |
...
| 约束 | - 题干必须**显式涉及 ≥ 2 个 Concept**（如"Embedding 与检索的关系是？"）<br>- 全部为 Choice / Sorting（不在 Challenge 阶段引入 Fill Blank，避免负担突增）<br>- 题数 ∈ [3, 5]<br>- 反馈机制与 FR-04 一致 |
| 验收 | AC1: Challenge 题在所有 Concept 完成后解锁<br>AC2: 每道题题干涉及 ≥ 2 个 Concept<br>AC3: 题数 ∈ [3, 5]<br>AC4: 答错重试机制与普通 Quiz 一致 |
...
| 输入 | 全部 `AttemptRecord` |
| 输出 | `Mastery` 对象 |
| 约束 | - MVP 不使用 IRT/BKT/DKT，仅用简单答对率<br>- `conceptMastery` = 该 Concept 内所有 Quiz 的"首次答对率"（重试不计入分子，但计入分母以反映难度）<br>- `moduleCompletion` = 已完成的 Quiz 数 / 总 Quiz 数（含费曼 Step 1-6）<br>- `feynmanScore` = Step 6 的 Rubric 总分<br>- 掌握度实时更新，每答一题刷新<br>- 数据持久化到 LocalStorage（FR-08） |
...
| 约束 | - LocalStorage key 命名：`alc:module:{id}` / `alc:mastery:{moduleId}` / `alc:attempts:{quizId}`<br>- 单 Module 数据总量 ≤ 5MB（LocalStorage 上限）<br>- 提供"清空进度重学"入口<br>- 不做跨设备同步（MVP） |
| 验收 | AC1: 刷新页面后进度恢复<br>AC2: 关闭浏览器重开后进度恢复<br>AC3: "清空进度"按钮可重置 Module 到初始状态<br>AC4: LocalStorage 数据超 4.5MB 时提示用户 |
...
[Module 导言页] "完成本模块后，你能……"
  ...
  ▼
[Concept A 学习循环] ─────────────────┐
  │  Quiz 1 (Level 1 Choice)          │
  │  → 反馈 → 下一题                  │
  │  Quiz 2 (Level 1 Choice)          │
  │  ...                              │
...
Module Agent (聚类为单 Module，生成 intro/goal)
  │  → 显示进度 65%
  ▼
Mission Agent (为每个 Concept 编排 Quiz Series)
  │
  ▼
Quiz Agent ×N (并行生成 8-15 题/Concept)
  │  → 显示进度 80%
  ▼
...
| NFR-P2 | 单题 Feedback Agent 响应 | P50 ≤ 800ms, P95 ≤ 1.5s |
| NFR-P3 | 答错后新题生成耗时 | P95 ≤ 3s |
...
| NFR-U3 | 离线可用 | 编译完成后，学习循环可在纯离线状态进行（Feedback Agent 除外，需联网） |
...
| NFR-R2 | AI Agent 调用失败处理 | 单 Agent 失败自动重试 2 次；仍失败则显示明确错误，不崩溃 |
| NFR-R3 | 进度丢失防护 | 每次作答后立即写 LocalStorage；崩溃后可从最近一题恢复 |
| NFR-R4 | JSON Schema 校验 | 所有 Agent 输出强制 JSON Schema 校验，失败则重试 |

### 6.4 成本（NFR-C）

| 编号 | 指标 | 目标 |
|------|------|------|
| NFR-C1 | 单次编译 LLM 调用成本 | ≤ $0.20（按 GPT-4o-mini 或同等模型估算） |
| NFR-C2 | 单次完整学习（含编译+全 Module）总成本 | ≤ $0.30 |
...
| NFR-S2 | LLM API Key 管理 | 用户自带 Key（OpenAI / Anthropic / Ollama 本地），不存储在服务端 |
...
| 输出 JSON Schema | 见规格书 [§7.2 Concept Agent Prompt](./Product-Specification.md#concept-agent-prompt) |
...
| 项 | 内容 |
|----|------|
| 输入 | 单个 Quiz 占位符 + 对应 Concept |
| 输出 | 完整 `Quiz` 对象：`{ id, conceptId, ladderLevel, expressionLevel, interactionType, stem, options[], answer, explanation, distractors[] }` |
| 约束 | - `stem` 清晰，用户无需思考"题目要我做什么"<br>- Choice 题：4 选项，1 正确 + 3 干扰<br>- 干扰项必须 **plausible but wrong**（来自常见误解或相近概念），不允许荒谬选项<br>- 正确答案不能通过常识排除法猜出<br>- `explanation` 同时说明"为什么对"和"为什么错"<br>- `distractors` 字段列出干扰项特征，用于答错后生成同类新题<br>- 输出 JSON 符合规格书 [§7.2 Quiz Agent Prompt](./Product-Specification.md#quiz-agent-prompt) |
| 重试场景约束 | 当用于答错重试生成新题时：保留 conceptId/ladderLevel/interactionType/expressionLevel；更换 stem 与至少 2 个 distractors |
...
| 约束 | - Step 1-4：Choice，4 选项<br>- Step 1 题干："如果让你解释 {goal}，第一句话应该是什么？"<br>- Step 2 题干："接下来应该解释什么方向？"<br>- Step 3 题干："如果对方没懂，你会举什么例子？"<br>- Step 4 题干："下面四个完整解释，哪一个最好？" — 4 选项质量分层（优秀/良好/一般/错误）<br>- Step 5：Fill Blank，"请补充一句话：____"，空白处需填 ≤ 30 字<br>- `finalPrompt`：基于 `module.goal` 的开放任务<br>- `rubric`：3-5 个关键点，每点 ≤ 20 字，必须是 Concept 核心而非细节<br>- 输出 JSON 符合规格书 [§7.2 Feynman Agent Prompt](./Product-Specification.md#feynman-agent-prompt) |

### 7.8 Feedback Agent（运行时）

| 项 | 内容 |
|----|------|
| 输入 | `Quiz` + `userAnswer` |
| 输出 | `{ score, gaps[], next_action, feedback_text }` |
| 约束 | - `score` ∈ [0, 100]；MVP 用阈值：Choice/Sorting 全对=100、错=0；Fill Blank 关键词全命中=100、部分=50、无=0<br>- `gaps`：Choice/Sorting 答错时列出用户忽略的关键点（≤ 2 条）<br>- `next_action` ∈ `advance` / `retry`；score ≥ 80 → advance，< 80 → retry<br>- `feedback_text` ≤ 50 字，鼓励性语气<br>- 输出 JSON 符合规格书 [§7.2 Feedback Agent Prompt](./Product-Specification.md#feedback-agent-prompt运行时) |
| 性能 | P95 响应 ≤ 1.5s |
...
继承规格书 [§6 数据结构](./Product-Specification.md#第六章-数据结构)。MVP 实现以下 TypeScript 接口（可直接用于前端类型定义）：
...
| 反馈态 | 题干保留 + 选项标注对错（轻量色彩，非鲜红）+ 解释文案 + [下一题] |
| Module Challenge 页 | 视觉上与 Concept 学习页区分（如换主色调），强调"综合挑战"感 |
| Feynman 步骤页 | 步骤指示器（1/6 ... 6/6）+ 与 Quiz 页类似交互 |
| Feynman 最终输出页 | 大文本输入框 + 字数提示 + [提交]；提交后显示 Rubric 评分卡 + gaps + 示例范文 |
| 完成页 | 祝贺文案 + Mastery 卡片（各 Concept 掌握度）+ [重新学习] |
...
- **MVP 策略**：用**静态编排**（Mission Agent 预先生成层级递进的 Quiz）+ **答错重试机制**（同类型新题）近似实现，而非实时自适应
...
- **例外**：US-12 [S] 作为 Should 级别，可在 V1.1 通过 A/B 测试验证"显示层级是否提升还是降低动机"
...
```
page_view: { page: 'home' | 'import' | 'compiling' | 'overview' | 'learn' | 'feynman' | 'done' }
...
quiz_attempt: { quizId, ladderLevel, expressionLevel, score, attemptCount, durationMs }
...
继承规格书 [§5.1 不包含](./Product-Specification.md#不包含)。MVP **明确不做**：
...
| Fill Blank 答案匹配误判率高 | 中 | 双策略匹配（§10.5）；Feedback Agent 兜底语义判断 |
| 单次编译成本超 $0.30 | 中 | 选择低成本模型（GPT-4o-mini / Claude Haiku / Ollama 本地）；并行 Quiz Agent 调用 |

### 13.2 技术依赖

| 依赖 | 说明 |
|------|------|
| LLM API | OpenAI兼容 / Anthropic兼容 / 本地Ollama 三选一，用户自带 Key |
| Next.js + React | 前端框架 |
| Vercel | 静态部署 |
...
1. **LLM 提供商默认值**：MVP 是否默认使用某一特定 LLM（如 OpenAI）？还是允许用户三选一？（建议：默认 OpenAI，提供 Ollama 选项，文档说明 Anthropic 兼容方式）
   1. 供应商自带DeepSeek / GLMCodingPlan
...
| **P2: 永远降低表达成本** | FR-03 表达层级分布（Level 1 ≥ 60%）；FR-04 答错不惩罚；§9.1 全局交互原则 |
| **P3: 每一道题都让用户成功** | FR-03 前 2 题强制 Level 1；FR-04 同类型新题重试；§9.3 不用鲜红叉号 |
| **P4: 不让用户思考"如何回答"** | FR-03 题干清晰约束；Quiz Agent Prompt 约束；§9.1 最少认知负荷 |
| **P5: 输出自由度逐渐增加** | FR-03 表达层级曲线；FR-06 费曼 6 步从 Choice 到完整输出 |
...
> **文档结束**
>
> 本 PRD 定义了 AI Learning Compiler V1 MVP 的可实施需求。任何对本 PRD 的修改应：
> 1. 不违反规格书第一章五条产品原则
> 2. 不违反规格书第九章学习心理学基础
> 3. 经过产品 + 工程 + 设计三方评审
>
