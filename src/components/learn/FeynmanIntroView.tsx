    ...
    <div className="text-fg-primary">
      <div className="max-w-2xl mx-auto px-6 py-16 space-y-8">
        <div className="space-y-2">
          <p className="text-xs text-fg-quaternary uppercase tracking-wider">费曼任务</p>
        ...
        {/* Rubric preview */}
        <div className="space-y-2">
          <p className="text-xs text-fg-quaternary uppercase tracking-wider">评分维度</p>
          <ul className="space-y-1">
            {feynmanTask.rubric.map((point) => (
              <li key={point} className="text-sm text-fg-secondary flex items-start gap-2">
                <span className="text-fg-quaternary mt-0.5">-</span>
                <span>{point}</span>
              </li>
            ))}
          </ul>
        ...
        <div className="border-l-2 border-border-strong pl-4 py-1">
          <p className="text-xs text-fg-quaternary mb-1">最终任务</p>
          <p className="text-sm text-fg-secondary">{feynmanTask.finalPrompt}</p>
        </div>
