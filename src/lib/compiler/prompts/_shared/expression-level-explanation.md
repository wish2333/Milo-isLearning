...
- **交互类型**：选择题为主，少数题目填入 1-3 个关键词
- **用户负担**：中等负担（输入 1-3 个词）
- **使用比例**：≤ 20%
- **典型场景**：定义补全（"X 的核心思想是 ____"）、关键术语填空
- **关键约束**：
  - 空白处**只允许填关键词**，不允许填句子
  - 答案匹配采用双策略：标准化精确匹配 + LLM 语义判断（PRD §10.5）
  - 空白处长度限制 ≤ 30 字
- **MVP 不包含短句补全**（规格书 Level 4），V1.1 再引入
...
- `expressionLevel: 3` → Fill Blank（关键词填空，空白处 ≤ 30 字）
