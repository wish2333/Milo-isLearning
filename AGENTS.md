...
| **Topic 入口恢复进度 (V2.0.1)** | `runtime/enter-module.ts` + 5 处调用点（TopicCard/TopicTransitionView×3/ShowcaseHome）                                                                           | `enterModule({ moduleId, allowResume })` 统一入口；调 `setModule` + `resumeModule`/`startModule`。**禁止** Topic 入口直接调 `startModule`（显式重做除外）       |
| **模块细粒度进度 (V2.0.1)**     | `runtime/module-progress.ts` + 题库/主题卡片                                                                                                                     | `computeModuleProgress(module, progress)` 纯函数；concept 维度百分比 + label（如 "概念 2/5"）；不计算"已答题数"（quizIndex 可能落在 reviewSlot）                |
...
### Topic re-entry & fine-grained progress (v2.0.1)

| Symbol                  | Type | Location                     | Role                                                                                                                                                                            |
| ----------------------- | ---- | ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `resumeModule`          | fn   | `state/progress-store.ts`    | V2.0.1 fix: 三级 fallback (LS per-module snapshot → 全局 Zustand blob → startModule 语义)；`stage.kind==='done'` 一律走 startModule；恢复时清空 `feynmanAttempt` (已知不满足项) |
| `enterModule`           | fn   | `runtime/enter-module.ts`    | V2.0.1 fix: Topic 入口统一 helper；`allowResume=true` 调 resumeModule，`false` 调 startModule；返回 boolean，调用方负责 router.push                                             |
| `computeModuleProgress` | fn   | `runtime/module-progress.ts` | V2.0.1 S4: 纯函数；按 concept 维度返回 `ModuleProgressInfo`（completedConcepts/conceptPercent/label/started/done）；不计算"已答题数"                                            |
